...
if __name__ == '__main__':
    ...
    case_list = ['Post_Gibbon', 'Post_NWM', 'Post_Oran', 'Post_OWM']
    case_list = ['Before_Marmoset']
    for case in case_list:
        path = './withOutgroup/' + case + '/'
        path = './noOutgroup/' + case + '/'
        pair_file = path + case + '_List.txt'
        pairs = list()
        with open(pair_file, 'r') as f:
            for line in f.readlines():
                pairs.append(line.replace('\n','').split('_'))

##        if case == 'Before_Marmoset':
##            pairs.remove(['ENSG00000104827', 'ENSG00000267335'])
##            pairs.remove(['ENSG00000189052', 'ENSG00000267335'])
##            pairs.remove(['ENSG00000196337', 'ENSG00000267335'])
##            pairs.remove(['ENSG00000205777', 'ENSG00000275113'])
##            pairs.remove(['ENSG00000215269', 'ENSG00000275113'])
##            pairs.remove(['ENSG00000215274', 'ENSG00000275113'])
##            pairs.remove(['ENSG00000216649', 'ENSG00000275113'])
##            pairs.remove(['ENSG00000224659', 'ENSG00000275113'])
##            pairs.remove(['ENSG00000227488', 'ENSG00000275113'])
##            pairs.remove(['ENSG00000236362', 'ENSG00000275113'])
##            pairs.remove(['ENSG00000237671', 'ENSG00000275113'])
##            pairs.remove(['ENSG00000239862', 'ENSG00000250036'])
##            pairs.remove(['ENSG00000267335', 'ENSG00000267631'])
        ...
        for pair in pairs:
            ...
            translateDNAtoAA(path + '_'.join(pair) + '/' + '_'.join(pair) + '.fa',
                             path + '_'.join(pair) + '/' + '_'.join(pair) + '_AA.fa')
            ...
            translateAAAlignmentDNAAlignment(path + '_'.join(pair) + '/' + '_'.join(pair) + '_AA_MAFFT_formated.fa',
                                             path + '_'.join(pair) + '/' + '_'.join(pair) + '.fa',
                                             path + '_'.join(pair) + '/' + '_'.join(pair) + '_MAFFT.fa')

            GapRemovedFasta(processAlignment(path + '_'.join(pair) + '/' + '_'.join(pair) + '_MAFFT.fa'),
                            path + '_'.join(pair) + '/' + '_'.join(pair) + '_input.fasta')
