...
def copy_and_remove_gaps(original_file, new_file):
...
if __name__ == '__main__':
    path = '/Users/Xiang/GitFolders/PrimateMultigeneFamilySearch/MAFFT_alignment/'
    pairs = []
    with open('../GeneFamilyList_LessAccurate.txt', 'r') as f:
        for line in f.readlines():
            pairs.append(line.replace('\n','').split('_'))
    pairs.remove(['CNTN6', 'CNTN4'])
    os.chdir(path)
...
##        cp_cmd = ['cp',
##                  '../GeneFamilies/' + '_'.join(pair) + '.fasta',
##                  './' + '_'.join(pair) + '/' + '_'.join(pair) + '.fasta']
        subprocess.call(mkdir_cmd)
##        subprocess.call(cp_cmd)

        original_file = '../GeneFamilies/' + '_'.join(pair) + '.fasta'
        new_file = path + '_'.join(pair) + '/' + '_'.join(pair) + '.fasta'
        copy_and_remove_gaps(original_file, new_file)
        
        translateDNAtoAA(new_file,
                         path + '_'.join(pair) + '/' + '_'.join(pair) + '_AA.fa')
